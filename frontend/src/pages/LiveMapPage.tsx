...
export const LiveMapPage: React.FC = () => {
  ...
  const handleUpdate = async (zoneName: string, action: 'ENTER' | 'EXIT') => {
    ...
    try {
      ...
      setActionMessage(`Successfully ${action === 'ENTER' ? 'entered' : 'exited'} ${zoneName}`);
      await fetchCrowdStatus();
    } catch (err: any) {
      setActionError(err?.response?.data?.message || 'Failed to update location');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-50 to-cyan-50">
      ...
      <div className="p-8">
        ...
        {actionMessage && <div className="bg-green-100 border border-green-400 text-green-800 px-4 py-3 rounded mb-6">{actionMessage}</div>}

        {/* Campus Map Visualization */}
        <div className="bg-white rounded-lg shadow-2xl p-8 mb-8">
          <svg viewBox="0 0 1000 600" className="w-full border border-gray-300 rounded">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="flex items-center gap-3">
              ...
              <span className="text-gray-700">
                <span className="font-semibold">LOW</span> - Less than 50% capacity
              </span>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-6 h-6 rounded-full bg-yellow-500"></div>
              <span className="text-gray-700">
                <span className="font-semibold">MEDIUM</span> - 50-80% capacity
              </span>
            </div>
            <div className="flex items-center gap-3">
          ...
          {zones.map((zone) => (
            ...
            >
              ...
              <div className="space-y-2 text-sm text-gray-600">
                ...
                <p>
                  <span className="font-semibold">Occupancy:</span> {zone.currentCount}/{zone.capacity}
                </p>
                <p>
                  <span className="font-semibold">Percentage:</span> {zone.occupancyPercentage.toFixed(1)}%
                </p>
                <p>
              ...
              <div className="mt-4 flex gap-3">
                ...
                >
                  {updatingZone === zone.name + 'ENTER' ? 'Entering...' : 'Enter'}
                </button>
                <button
                ...
                >
                  {updatingZone === zone.name + 'EXIT' ? 'Exiting...' : 'Exit'}
                </button>
              </div>
            </div>
          ))}
